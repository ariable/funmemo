...
export function buildChineseSummaryPrompts(transcript: Transcript, format: SummaryOutputFormat = "markdown") {
  ...
  const baseRules = [
    ...
    "- 使用清晰、正式、通用的书面中文，不要使用黑话、口语化表达或行业俚语，例如「缺口」「卡点」「抓手」等。",
...
export async function generateMeetingSummary(
  ...
  const promptTokensEstimate = Math.ceil((systemPrompt.length + userPrompt.length) / 2);
  console.log(
    `[summary] 开始请求 LLM｜model=${llmModel}｜format=${format}｜prompt≈${promptTokensEstimate}字符｜endpoint=${endpoint}`,
  );
  ...
  try {
    ...
    console.error(`[summary] 请求异常｜耗时=${elapsed}s｜error=${message}`);
    throw new Error(`纪要生成失败：请求 LLM 异常（${message}）`);
  }
  clearTimeout(timeout);
  ...
  if (!response.ok) {
    ...
    console.error(`[summary] LLM 返回错误｜status=${response.status}｜耗时=${elapsed}s｜body=${message.slice(0, 500)}`);
  ...
  console.log(
    `[summary] LLM 返回｜耗时=${elapsed}s｜finish_reason=${finishReason}｜content长度=${content?.length ?? 0}字符` +
      (usage ? `｜tokens: prompt=${usage.prompt_tokens} completion=${usage.completion_tokens} total=${usage.total_tokens}` : ""),
  );

  if (!content) {
    console.error(`[summary] 模型返回内容为空｜raw choices=${JSON.stringify(data.choices ?? null).slice(0, 500)}`);
    throw new Error("纪要生成失败：模型返回内容为空，请重试。");
  }

  if (finishReason === "length") {
    console.warn(`[summary] 模型输出被截断（finish_reason=length），当前 max_tokens=${body.max_tokens}`);
  }
